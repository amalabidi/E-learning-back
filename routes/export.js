...
router.post("/modele", async (req , res)=>{
    ...
    try{
        ...
        const modele = new  Modele({clientAttributes,dossierAttributes,factureAttributes,workshopAttributes}) ; 
...
router.get("/exportdata", async (req , res)=>{
     ...
     try {
        ...
        const dossier = await Dossier.findOne({"_id":dossierId[i]}).populate("facturation").populate("idWorkshop").populate("client") ;  
        var result={};
        ...
        for (j in factureAttributes){    
            result[factureAttributes[j]]=dossier["facturation"][factureAttributes[j]] ;
        }
        
        for (j in workshopAttributes){
            result[workshopAttributes[j]]=dossier["idWorkshop"][workshopAttributes[j]] ;
        }
...
router.get("/exportdataMSC", async (req , res)=>{
    // tableau de dossierId
    const dossierId = req.body.dossierId ; 
    const dossierAttributes = ["numeroEDOF","workshopBeginDate","workshopEndDate"] ; 
